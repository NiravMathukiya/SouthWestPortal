...
import { create, delete_recode, find_one, ge_all_booking, update } from "../repositories/booking";
...
export const delete_booking = async (
  request: NextRequest,
  model: string
): Promise<NextResponse> => {
  try {
...
export const read_booking = async (
  request: NextRequest,
  model: string
): Promise<NextResponse> => {
  try {
...
export const list_booking = async (
  request: NextRequest,
  model: string
): Promise<NextResponse> => {
  try {
    ...
    if (searchParams.get("filter_username")) {
      filters.username = { contains: searchParams.get("filter_username") || "" };
    }
    if (searchParams.get("filter_first_name")) {
      filters.first_name = { contains: searchParams.get("filter_first_name") || "" };
    }
    if (searchParams.get("filter_last_name")) {
      filters.last_name = { contains: searchParams.get("filter_last_name") || "" };
    }
    ...
    if (searchParams.get("filter_status")) {
      filters.status = searchParams.get("filter_status") || undefined;
    }
    if (searchParams.get("filter_sub_role")) {
      filters.sub_role = searchParams.get("filter_sub_role") || undefined;
    }
    if (searchParams.get("filter_attachments")) {
      filters.attachments = searchParams.get("filter_attachments") || undefined;
    }
    ...
    const data = await ge_all_booking();
    return sendResponse({ data: data }, 200);
  } catch (error) {
    console.log(error)
