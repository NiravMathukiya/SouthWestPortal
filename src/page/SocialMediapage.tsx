...
const SocialMediaPage: React.FC = () => {
    ...
    const fetchRequests = async (
        ...
        try {
            ...
            const res = await axios.get(`/api/socialmedia?${params.toString()}`);
            const data = res.data;

            if (data.success && data.data) {
                setCurrentPage(Number(data.data.pagination.page));
                setTotalPages(data.data.pagination.pages);
                setItemsPerPages(data.data.pagination.limit);
    ...
    const handleDelete = async () => {
        ...
        setDeleting(true);
        try {
            const res = await axios.delete("/api/socialmedia", {
    ...
    return (
        <div>
            {/* <Header title="Communication Request : Social Media" handleDelete={handleDelete} /> */}

            <ExportAddDeleteHeader title="Communication Request : Social Media" handleDelete={handleDelete} HandleExcelExport={handleExcelExport} HandlePdfExport={handlePdfExport} handleAddUrl="/admin/announcements/new" />
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 py-3 bg-gray-50 border-b gap-2">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center">
                    ...
                    Communication Request : Social Media
                </h2>
                <button
